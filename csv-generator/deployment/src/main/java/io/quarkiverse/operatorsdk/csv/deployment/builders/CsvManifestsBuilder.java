...
public class CsvManifestsBuilder extends ManifestsBuilder {
    ...
    public byte[] getYAMLData(List<ServiceAccount> serviceAccounts, List<ClusterRoleBinding> clusterRoleBindings,
        ...
        clusterRoleBindings.forEach(clusterRoleBinding -> handleClusterPermission(clusterRoleBinding,
                findRules(clusterRoleBinding, clusterRoles, roles), defaultServiceAccountName, groupToCRInfo, installSpec));
        roleBindings.forEach(roleBinding -> handlerPermission(roleBinding,
                findRules(roleBinding, clusterRoles, roles), defaultServiceAccountName, installSpec));
    ...
    private void handlerPermission(RoleBinding roleBinding,
            List<PolicyRule> rules,
            String defaultServiceAccount,
            NamedInstallStrategyFluent.SpecNested<ClusterServiceVersionSpecFluent.InstallNested<ClusterServiceVersionFluent.SpecNested<ClusterServiceVersionBuilder>>> installSpec) {
        if (roleBinding != null && !rules.isEmpty()) {
            installSpec
                    .addNewPermission()
                    .withServiceAccountName(findServiceAccountFromSubjects(roleBinding.getSubjects(), defaultServiceAccount))
                    .addAllToRules(rules)
                    .endPermission();
        }
    }

    private void handleClusterPermission(
    ...
    private List<PolicyRule> findRules(ClusterRoleBinding binding, List<ClusterRole> clusterRoles, List<Role> roles) {
        if (binding == null || binding.getRoleRef() == null) {
            return Collections.emptyList();
        }

        return findRules(binding.getRoleRef().getKind(), binding.getRoleRef().getName(), clusterRoles, roles);
    }

    private List<PolicyRule> findRules(RoleBinding binding, List<ClusterRole> clusterRoles, List<Role> roles) {
        if (binding == null || binding.getRoleRef() == null) {
            return Collections.emptyList();
        }
    ...
    private List<PolicyRule> findRules(String roleRefKind, String roleRefName, List<ClusterRole> clusterRoles,
            List<Role> roles) {
