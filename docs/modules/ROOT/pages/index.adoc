...
Make sure you have installed the https://github.com/operator-framework/operator-registry[opm] command tool and have connected to a Kubernetes cluster with the OLM installed.

1. Generate the Operator image and bundle manifests

Quarkus provides https://quarkus.io/guides/container-image[several extensions] to build the container image. For example, the https://github.com/quarkiverse/quarkus-operator-sdk/tree/main/samples/joke#deployment[Joke] sample uses the https://quarkus.io/guides/container-image#jib[Quarkus Jib container image extension] to build the image. So, you first need to configure one of these extensions at your choice.
...
This extension generates the Operator bundle manifests at `target/bundle`.
...
[NOTE]
====
Find more information about channels and packages in https://olm.operatorframework.io/docs/best-practices/channel-naming/#channels[this link].
====
...
An Operator Bundle is a container image that stores Kubernetes manifests and metadata associated with an operator. You can find more information about this in https://olm.operatorframework.io/docs/tasks/creating-operator-bundle/[here].
...
[NOTE]
====
We need to install the catalog source at the Kubernetes namespace where we installed OLM, which by default is `operators`.
====
...
[source,shell script]
----
kubectl get pods -n <K8S NAMESPACE> --selector=olm.catalogSource=my-catalog-source
----

4. Install your operator via OLM

OLM deploys operators via https://olm.operatorframework.io/docs/tasks/install-operator-with-olm/#install-your-operator[subscriptions]. Creating a  `Subscription` will trigger the operator deployment. You can simply create the `Subscription` resource that contains the operator name and channel to install by running the following command:
...
[source,shell script]
----
kubectl get csv -n operators my-operator-name
----
